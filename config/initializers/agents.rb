...
Rails.application.configure do
  # Ensure the custom config namespace exists
  config.x.agents ||= ActiveSupport::OrderedOptions.new

  config.x.agents.AGENTS_CONFIG = {
    'agent_1' => { # Vince
  ...
  # Ensure all expected agent keys are present
  expected_agents = ['agent_1', 'agent_2', 'agent_3']
  actual_agents = config.x.agents.AGENTS_CONFIG.keys.sort

  unless actual_agents == expected_agents &&
         config.x.agents.AGENTS_CONFIG.values.all? do |v|
           v.is_a?(Hash) && 
           v[:instructions].is_a?(String) && !v[:instructions].blank? &&
           v[:image_path].is_a?(String) && !v[:image_path].blank? &&
           v[:voice_id].is_a?(String) && !v[:voice_id].blank?
         end
    Rails.logger.error(
      "AGENTS_CONFIG structure is invalid or incomplete in config/initializers/agents.rb. " \
      "Expected agents: #{expected_agents.inspect}, Actual: #{actual_agents.inspect}. " \
      "Check instructions, image_path, and voice_id for all agents."
    )
    # Optionally raise an error during boot in non-production environments
    # raise "Invalid AGENTS_CONFIG structure" unless Rails.env.production?
  end

  # For debugging during startup (check your development.log or console output):
  if defined?(Rails::Server) || defined?(Sidekiq) # Log if server or sidekiq is starting
    Rails.logger.info "--- AGENTS.RB INITIALIZER ---_MODIFIED"
    Rails.logger.info "Rails.application.config.x.agents.AGENTS_CONFIG defined? #{config.x.agents.AGENTS_CONFIG.present?}"
    if config.x.agents.AGENTS_CONFIG.present?
      Rails.logger.info "Rails.application.config.x.agents.AGENTS_CONFIG keys: #{config.x.agents.AGENTS_CONFIG.keys.inspect}"
      Rails.logger.info "Config for 'agent_3': #{config.x.agents.AGENTS_CONFIG['agent_3'].inspect}"
      Rails.logger.info "Instructions for 'agent_3': #{config.x.agents.AGENTS_CONFIG.dig('agent_3', :instructions).inspect}"
    else
      Rails.logger.warn "Rails.application.config.x.agents.AGENTS_CONFIG is empty or not loaded correctly!"
    end
    Rails.logger.info "--- END AGENTS.RB INITIALIZER ---_MODIFIED"
  end
end