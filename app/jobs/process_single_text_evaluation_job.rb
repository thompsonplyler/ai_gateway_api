...
class ProcessSingleTextEvaluationJob < ApplicationJob
  ...
  # Define personalities/instructions for the Run, adding style constraints and example placeholders
  AGENT_INSTRUCTIONS = {
    'agent_1' => <<~PROMPT.strip,
  ...
  def perform(text_evaluation_id)
    ...
    agent_instructions = AGENT_INSTRUCTIONS[text_evaluation.agent_identifier]

    unless assistant_id && agent_instructions
      msg = "Missing OpenAI Assistant ID or instructions for agent #{text_evaluation.agent_identifier}"
